...
compile.encoding = function (encoding, stats) {
  ...
  if(!hasRow && !hasCol) {
	  ...
	  if(lineType) {
		  var pointMark = marks['point'];
		  var pointDefs = marks.def(pointMark, encoding, layout, style);
		  group.marks.push(pointDefs[0]);
	  }
	
	  encoding._enc.text.type = encoding._enc.y.type;
	  encoding._enc.text.name = encoding._enc.y.name;
	  encoding._enc.text.aggr = encoding._enc.y.aggr;
	  
	  var textMark = marks['text'];
	  var textDefs = marks.def(textMark, encoding, layout, style);
	  // group.marks.push(textDefs[0]);

  }

  if(encoding.marktype() === 'pie') {
...
/*
 * HACK to set chart size NOTE: this fails for plots driven by derived values (e.g., aggregates) One solution is to update Vega to support auto-sizing In the meantime, auto-padding (mostly) does the trick
 */
...
marks.pie = {
  ...
  stack: false,
  prop: pie_props,
  requiredEncoding: ['x', 'y', 'color'],
  supportedEncoding: {row: 1, col: 1, x: 1, y: 1, color: 1, text : 1}
};
...
marks.point = {
  type: 'symbol',
  prop: point_props,
  supportedEncoding: {row: 1, col: 1, x: 1, y: 1, size: 1, color: 1, alpha: 1, shape: 1, detail: 1, text: 1}
};
...
function pie_props(e, layout, style) {
	
  var p = {};

  // x
  // if (e.isMeasure(X)) {
    p.x = {group: "width", mult: 0.5};
  // }

  // y
  // if (e.isMeasure(Y)) {
    p.y = {group: "height", mult: 0.5};
  // }

  p.startAngle = { field : "startAngle" };
  p.endAngle = { field : "endAngle" };
  p.innerRadius = { value: e.enc(SIZE).value };
  p.outerRadius = { value: p.innerRadius.value + 50 };
  ...
  if (e.has(COLOR)) {
    ...
    p.fill = {scale: COLOR, field: e.field(X)};
  }
...
/**
 * @param {Object}
 *            data data in JSON/javascript object format
...
/*
 * Most fields that use ordinal scale are dimensions. However, YEAR(T), YEARMONTH(T) use time scale, not ordinal but are dimensions too.
 */
...
/**
 * For encoding, use encoding.isDimension() to avoid confusion. Or use Encoding.isType if your field is from Encoding (and thus have numeric data type). otherwise, do not specific isType so we can use the default isTypeName here.
 */
...
/**
 * For encoding, use encoding.cardinality() to avoid confusion. Or use Encoding.isType if your field is from Encoding (and thus have numeric data type). otherwise, do not specific isType so we can use the default isTypeName here.
 */
...
schema.marktype = {
  type: 'string',
  enum: ['point', 'tick', 'bar', 'line', 'area', 'circle', 'square', 'text', 'pie']
};
...
var axisMixin = {
  type: 'object',
  supportedMarktypes: {point: true, tick: true, bar: true, line: true, area: true, circle: true, square: true, pie: true},
  properties: {
    axis: {
      type: 'object',
      properties: {
...
var sizeMixin = {
  type: 'object',
  supportedMarktypes: {point: true, bar: true, circle: true, square: true, text: true, pie : true},
  properties: {
    value: {
      type: 'integer',
      default: 30,
...
var colorMixin = {
  type: 'object',
  supportedMarktypes: {point: true, tick: true, bar: true, line: true, area: true, circle: true, square: true, text: true, pie : true},
  properties: {
    value: {
      type: 'string',
      role: 'color',
 ...
 * 
 * @param noaugment
 *            determine whether new object should be added f or non-existing properties along the path
 */
